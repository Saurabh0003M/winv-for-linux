...
#   tests/run.sh fedora:rawhide    the newest GNOME
...
set -euo pipefail
TARGET=${1:?"usage: tests/run.sh ubuntu:24.04 | ubuntu:26.04 | fedora:rawhide | ..."}
DISTRO=${TARGET%%:*}
VERSION=${TARGET#*:}
...
podman image exists "$IMAGE" ||
    podman build -f "$TESTS/containers/$DISTRO.Containerfile" --build-arg VERSION="$VERSION" -t "$IMAGE" "$TESTS/containers"
rm -rf "$OUT"
mkdir -p "$OUT"
