...
// WinV for Linux — a Windows 11 style Win+V / Win+. panel for GNOME Shell 42.
...
const { Clutter, GdkPixbuf, GLib, Gio, Meta, Pango, Shell, St } = imports.gi;
...
class ClipboardPanel {
    ...
    _position(anchor) {
        ...
        const rect = new Meta.Rectangle({ x: Math.round(anchor.x), y: Math.round(anchor.y), width: 1, height: 1 });
    ...
    _icon(name) {
        if (!this._icons[name])
            this._icons[name] = Gio.icon_new_for_string(`${Me.path}/icons/${name}-symbolic.svg`);
    ...
    _build() {
        ...
        this._panel = new St.BoxLayout({ style_class: 'winv-panel', vertical: true, reactive: true });
        ...
        for (const tab of TABS) {
            ...
            const box = new St.BoxLayout({ vertical: true, x_expand: true, y_align: Clutter.ActorAlign.CENTER });
            box.add_child(glyph);
    ...
    _buildPage(id) {
        const page = new St.BoxLayout({ vertical: true, style_class: 'winv-page', x_expand: true, y_expand: true });
        if (id === 'clipboard')
    ...
    _grid(entries, scroll) {
        ...
        const grid = new St.BoxLayout({ vertical: true, style_class: 'winv-grid', x_expand: true });
        const cells = [];
        let row = null;
    ...
    _buildClipboardPage(page) {
        ...
        const list = new St.BoxLayout({ vertical: true, style_class: 'winv-clip-list', x_expand: true });
        for (const item of history.items)
    ...
    _buildRecentPage(page) {
        ...
        const scroll = this._scroll();
        const box = new St.BoxLayout({ vertical: true, style_class: 'winv-recent', x_expand: true });
...
// var, not let: reachable from Looking Glass (Alt+F2, lg) as
